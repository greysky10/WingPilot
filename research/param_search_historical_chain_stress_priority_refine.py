...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a narrow stress-priority historical-chain search around the first "
            "stress-positive single-28DTE branch."
        )
    )
    parser.add_argument(
        "--bars-csv",
    ...
    parser.add_argument(
        "--output-dir",
        default=r".\corridor_outputs\fit_search\historical_chain_stress_priority_refine_search",
    ...
    for confidence in (0.60, 0.65):
        for max_hold in (2, 3):
            for take_profit in (0.10, 0.15, 0.20):
                for max_layers in (2, 3):
                    rows.append(
                        {
                            "bar_mode": "intraday_5m",
                            "profile_branch": "stress_priority_refine",
...
def config_name(params: dict[str, object]) -> str:
    return "_".join(
        [
            ...
            f"tp_{str(params['primary_take_profit_pct']).replace('.', 'p')}",
        ]
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print(f"Running {len(tasks)} stress-priority refinement cases with workers={max(1, int(args.workers))}.")
    rows: list[dict[str, object]] = []
