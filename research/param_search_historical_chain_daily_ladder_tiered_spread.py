...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Search daily long-DTE ladder additions with tiered spread caps: tighter for nearer expiries, wider for farther expiries."
        )
    )
    parser.add_argument(
        "--bars-csv",
    ...
    parser.add_argument(
        "--output-dir",
        default=r".\corridor_outputs\fit_search\historical_chain_daily_ladder_tiered_spread_search",
...
def parameter_grid() -> list[dict[str, object]]:
    ...
    stack_profiles = [
        {"profile_dte": "ladder_21_28", "dte_min": 21, "dte_max": 35, "default_dte": 21, "layer_dte_targets": (21, 28), "max_layers_values": (4, 6)},
        {"profile_dte": "ladder_28_35", "dte_min": 21, "dte_max": 35, "default_dte": 28, "layer_dte_targets": (28, 35), "max_layers_values": (4, 6)},
        {"profile_dte": "ladder_21_28_35", "dte_min": 21, "dte_max": 35, "default_dte": 21, "layer_dte_targets": (21, 28, 35), "max_layers_values": (6,)},
    ]
    spread_profiles = [
        {"spread_profile": "tight", "near_spread_dte_max": 14, "near_max_acceptable_option_spread": 0.10, "mid_max_acceptable_option_spread": 0.18, "far_spread_dte_min": 28, "far_max_acceptable_option_spread": 0.28},
        {"spread_profile": "base", "near_spread_dte_max": 14, "near_max_acceptable_option_spread": 0.12, "mid_max_acceptable_option_spread": 0.20, "far_spread_dte_min": 28, "far_max_acceptable_option_spread": 0.30},
        {"spread_profile": "loose", "near_spread_dte_max": 14, "near_max_acceptable_option_spread": 0.15, "mid_max_acceptable_option_spread": 0.22, "far_spread_dte_min": 28, "far_max_acceptable_option_spread": 0.35},
    ]
    entry_ends = ["09:45", "09:50", "10:00"]
    confidence_values = [0.50, 0.55]
                                        ...
                                        rows.append(
                                            {
                                                ...
                                                "profile_branch": "daily_ladder_tiered_spread",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print(f"Running {len(tasks)} tiered-spread daily-ladder cases with workers={max(1, int(args.workers))}.")
    rows: list[dict[str, object]] = []
    ...
    if not active.empty:
        ...
        for label, subset in {
            "best_exit_all": active[active["layer_exit_scope"] == "all"],
            "best_exit_individual": active[active["layer_exit_scope"] == "individual"],
        }.items():
            if subset.empty:
                continue
