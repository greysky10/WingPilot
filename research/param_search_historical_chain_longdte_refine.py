...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a tight refinement search around the profitable long-DTE intraday historical-chain branch, "
            "including side selection, event/gap filters, and stricter execution assumptions."
        )
    )
    parser.add_argument(
        "--bars-csv",
    ...
    parser.add_argument(
        ...
        help="Historical-chain CSV produced by run_massive_spx_flatfiles_backfill.py.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        default=r".\corridor_outputs\fit_search\historical_chain_longdte_refine_search",
...
def parameter_grid() -> list[dict[str, object]]:
    ...
    option_rights = ["call", "put", "auto"]
    widths = [
        {"profile_geometry": "w10", "butterfly_width": 10.0, "coverage_band_width": 20.0, "center_tolerance": 10.0, "recenter_threshold": 20.0},
        {"profile_geometry": "w12", "butterfly_width": 12.0, "coverage_band_width": 24.0, "center_tolerance": 12.0, "recenter_threshold": 24.0},
    ]
    entry_ends = ["09:50", "10:00", "10:10"]
    confidence_values = [0.50, 0.55, 0.60]
    momentum_values = [0.0020, 0.0025, 0.0030]
    close_dte_values = [2, 3]
    filter_profiles = [
        {"filter_profile": "none", "skip_event_days": False, "event_dates": (), "skip_gap_days": False, "max_entry_gap_pct": 0.0},
        {"filter_profile": "fomc_gap_1p0", "skip_event_days": True, "event_dates": FOMC_EVENT_DATES, "skip_gap_days": True, "max_entry_gap_pct": 0.010},
    ]
    execution_profiles = [
        {"execution_profile": "strict", "max_acceptable_option_spread": 0.20, "per_contract_slippage": 0.075, "stress_profile": "none"},
    ]

    for option_right in option_rights:
        for width in widths:
                    ...
                    for momentum in momentum_values:
                        for close_dte in close_dte_values:
                            for filters in filter_profiles:
                                for execution in execution_profiles:
                                    rows.append(
                                        {
                                            "bar_mode": "intraday_5m",
                                            "profile_branch": "longdte_refine",
                                            "profile_dte": "dte_21_35_28",
                                            "dte_min": 21,
                                            "dte_max": 35,
                                            "default_dte": 28,
                                            ...
                                            "option_right_preference": option_right,
                                            **filters,
                                            **execution,
...
def config_name(params: dict[str, object]) -> str:
    return "_".join(
        [
            ...
            f"right_{params['option_right_preference']}",
            ...
            str(params["filter_profile"]),
            str(params["execution_profile"]),
        ]
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print(f"Running {len(tasks)} long-DTE refinement cases with workers={max(1, int(args.workers))}.")
    rows: list[dict[str, object]] = []
    ...
    if not active.empty:
        ...
        for right in ["call", "put", "auto"]:
            subset = active[active["option_right_preference"] == right]
            if subset.empty:
                continue
            material_targets.append((f"best_{right}_score", subset.iloc[0].to_dict()))
