...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep body-strike offsets around the current recommended long-DTE single-28 stress-positive branch."
        )
    )
    parser.add_argument(
        "--bars-csv",
    ...
    parser.add_argument(
        ...
        help="Comma-separated body-strike offsets in points.",
    )
    parser.add_argument(
        "--output-dir",
        default=r".\corridor_outputs\fit_search\historical_chain_body_offset_search",
...
def parameter_grid(offsets: list[float]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for offset in offsets:
        rows.append(
            {
                "bar_mode": "intraday_5m",
                "profile_branch": "body_offset",
                "profile_dte": "single_28",
                "profile_geometry": "w10_single_28",
                "body_strike_offset_points": float(offset),
                "dte_min": 21,
                "dte_max": 35,
                "default_dte": 28,
                ...
                "primary_entry_min_center_confidence": 0.55,
                "primary_entry_max_momentum_pct": 0.001,
                "skip_entry_weekdays": ("mon", "fri"),
                "weekday_profile": "mon_fri",
...
def config_name(params: dict[str, object]) -> str:
    offset = str(params["body_strike_offset_points"]).replace("-", "m").replace(".", "p")
    return "_".join(
        [
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    offsets = [float(item.strip()) for item in str(args.offsets).split(",") if item.strip()]
    params_list = parameter_grid(offsets)
    params_by_name = {config_name(params): params for params in params_list}
    tasks = [(config_name(params), params, str(output_dir)) for params in params_list]
    print(f"Running {len(tasks)} body-offset cases with workers={max(1, int(args.workers))}.")
    rows: list[dict[str, object]] = []
    ...
    with ProcessPoolExecutor(
        ...
        for idx, future in enumerate(as_completed(future_map), start=1):
            ...
            print(
                ...
                f"offset={float(row['body_strike_offset_points']):+.1f} | "
    ...
    if not active.empty:
        best_stress_row = active.iloc[0].to_dict()
    else:
        best_stress_row = final.iloc[0].to_dict()
    best_params = params_by_name[str(best_stress_row["config_name"])]

    best_stress_dir = output_dir / "best_stress_active"
    materialize_run(
        bars_csv=bars_csv,
        ...
        params=best_params,
        destination=best_stress_dir,
    )

    best_base_dir = output_dir / "best_stress_active_base_match"
    materialize_run(
        bars_csv=bars_csv,
        historical_chain_path=historical_chain_path,
        start=str(args.start),
        end=str(args.end),
        params=base_match_params(best_params),
        destination=best_base_dir,
    )

    best_payload = {
        "best_stress_active": best_stress_row,
        "tested_offsets": offsets,
    }
    (output_dir / "best_offset_summary.json").write_text(json.dumps(best_payload, indent=2, default=str), encoding="utf-8")
