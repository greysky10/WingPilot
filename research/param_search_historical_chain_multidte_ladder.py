...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Search the long-DTE historical-chain winner with stacked multi-DTE layer ladders, "
            "including independent per-layer exits."
        )
    )
    parser.add_argument(
        "--bars-csv",
    ...
    parser.add_argument(
        "--output-dir",
        default=r".\corridor_outputs\fit_search\historical_chain_multidte_ladder_search",
...
def parameter_grid() -> list[dict[str, object]]:
    ...
    ladder_profiles = [
        {"profile_dte": "single_28", "dte_min": 21, "dte_max": 35, "default_dte": 28, "layer_dte_targets": (28,), "exit_scopes": ("all",)},
        {"profile_dte": "ladder_21_28", "dte_min": 21, "dte_max": 28, "default_dte": 21, "layer_dte_targets": (21, 28), "exit_scopes": ("all", "individual")},
        {"profile_dte": "ladder_21_35", "dte_min": 21, "dte_max": 35, "default_dte": 21, "layer_dte_targets": (21, 35), "exit_scopes": ("all", "individual")},
        {"profile_dte": "ladder_28_35", "dte_min": 28, "dte_max": 35, "default_dte": 28, "layer_dte_targets": (28, 35), "exit_scopes": ("all", "individual")},
        {"profile_dte": "ladder_21_28_35", "dte_min": 21, "dte_max": 35, "default_dte": 21, "layer_dte_targets": (21, 28, 35), "exit_scopes": ("all", "individual")},
        {"profile_dte": "ladder_14_21_28_35", "dte_min": 14, "dte_max": 35, "default_dte": 14, "layer_dte_targets": (14, 21, 28, 35), "exit_scopes": ("all", "individual")},
    ]
    entry_ends = ["09:45", "09:50", "10:00"]
    confidence_values = [0.55, 0.60]
    momentum_values = [0.0015, 0.0020]
    hold_sessions = [3, 5]
    close_dte_values = [2, 3]

    for ladder in ladder_profiles:
        for exit_scope in ladder["exit_scopes"]:
                                ...
                                rows.append(
                                    {
                                        ...
                                        "profile_branch": "multidte_ladder",
                                        "profile_dte": ladder["profile_dte"],
                                        "dte_min": int(ladder["dte_min"]),
                                        "dte_max": int(ladder["dte_max"]),
                                        "default_dte": int(ladder["default_dte"]),
                                        "layer_dte_targets": tuple(int(value) for value in ladder["layer_dte_targets"]),
                                        "layer_exit_scope": str(exit_scope),
                                        "max_active_butterfly_layers": len(ladder["layer_dte_targets"]),
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print(f"Running {len(tasks)} multi-DTE ladder cases with workers={max(1, int(args.workers))}.")
    rows: list[dict[str, object]] = []
    ...
    if not active.empty:
        ...
        for exit_scope in ["all", "individual"]:
            subset = active[active["layer_exit_scope"] == exit_scope]
            if subset.empty:
                continue
            material_targets.append((f"best_{exit_scope}_score", subset.iloc[0].to_dict()))
